...
class TestAuditLibrary:
    ...
    def test_slug_generation_automatic(self, organization):
        library = AuditLibrary.objects.create(
            name="Audit Library", organization=organization
        )

        assert library.slug == "audit-library"

    def test_organization_foreign_key(self, organization):
        library = AuditLibrary.objects.create(name="Library", organization=organization)
    ...
    def test_unique_together_organization_name(self):
        ...
        lib1 = AuditLibrary.objects.create(name="Library", organization=org1)
        lib2 = AuditLibrary.objects.create(name="Library", organization=org2)
        assert lib1.name == lib2.name, "Same name in different organizations"

        with pytest.raises(IntegrityError):
            AuditLibrary.objects.create(name="Library", organization=org1)
    ...
    def test_unique_together_both_constraints(self):
        ...
        lib1 = AuditLibrary.objects.create(name="My Library", organization=org1)
        original_slug = lib1.slug

        lib2 = AuditLibrary.objects.create(name="Another Library", organization=org1)
        assert lib2.slug != original_slug

        lib3 = AuditLibrary.objects.create(name="My Library", organization=org2)
        ...
        with pytest.raises(IntegrityError):
            AuditLibrary.objects.create(name="My Library", organization=org1)
    ...
    def test_organization_audit_libraries_relation(self, organization):
        library1 = AuditLibrary.objects.create(
            name="Library 1", organization=organization
        )
        library2 = AuditLibrary.objects.create(
            name="Library 2", organization=organization
        )
